...
# ---------- config ----------
T=int(os.environ.get("T","40")); SEQ_LEN=5
...
LAST=OUT/f"last_fall_T{T}_fb{FALL_BOOST}.pth"; BEST=OUT/f"best_fall_T{T}_fb{FALL_BOOST}.pth"
run_name=f"fall_L{SEQ_LEN}_T{T}_bs{BS}_ep{EPOCHS}_pf{P_FALL}_fb{FALL_BOOST}"
...
train_set=MADFallDataset(EXTRACT_DIR,CACHE_DIR,participants=TRAIN_P,activities=ACTIVITIES,
    fall_activity=FALL,seq_len=SEQ_LEN,T=T,samples_per_participant=SPP,p_fall=P_FALL,seed=0,split_name="train")
val_set=MADFallDataset(EXTRACT_DIR,CACHE_DIR,participants=VAL_P,activities=ACTIVITIES,
    fall_activity=FALL,seq_len=SEQ_LEN,T=T,samples_per_participant=SPP_VAL,p_fall=P_FALL,seed=1,split_name="val")
...
model=SResNestFPP(num_steps=T,num_classes=K).to(device)
