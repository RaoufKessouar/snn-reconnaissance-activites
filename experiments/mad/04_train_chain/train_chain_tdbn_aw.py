...
ADA_BN_BATCHES = 40
ACTIVITIES = [1, 3, 9]; SEQ_LEN = 3
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
OUT = EXP_ROOT / "04_train_chain"
LAST = OUT / "last_chain_tdbn_aw.pth"; BEST = OUT / "best_chain_tdbn_aw.pth"
run_name = f"madchain_TDBN_awLate_L{SEQ_LEN}_T{T}_bs{BS}_ep{EPOCHS}"
...
train_set = MADChainDataset(EXTRACT_DIR, CACHE_DIR, participants=TRAIN_P, activities=ACTIVITIES,
                            seq_len=SEQ_LEN, T=T, samples_per_class=1, split_name="train", augment=True, window_mode="late")
val_set = MADChainDataset(EXTRACT_DIR, CACHE_DIR, participants=VAL_P, activities=ACTIVITIES,
                          seq_len=SEQ_LEN, T=T, samples_per_class=3, split_name="val", window_mode="late")
